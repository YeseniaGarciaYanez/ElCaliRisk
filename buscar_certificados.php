...
// Si hay una búsqueda, agregar la cláusula WHERE para filtrar
if (!empty($busqueda)) {
    $busqueda = $conn->real_escape_string($busqueda);
    $query .= " WHERE c.codigo LIKE '%$busqueda%' OR tc.equipo LIKE '%$busqueda%'";
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . htmlspecialchars($row['codigoEquipo'] ?? 'N/A') . "</td>";
        echo "<td>" . htmlspecialchars($row['fechaExpedicion'] ?? 'N/A') . "</td>";
        echo "<td>" . htmlspecialchars($row['fechaVencimiento'] ?? 'N/A') . "</td>";
